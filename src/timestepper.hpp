...
namespace ASC_ode
{
  ...
  public:
    ImprovedEuler(std::shared_ptr<NonlinearFunction> rhs)
        : TimeStepper(rhs), m_vecf(rhs->dimF()), ytilde(rhs->dimX())
    {
    }
    void doStep(double tau, VectorView<double> y) override
    {
      /*
      ytilde = y + tau/2 * f(tn, y)
      ftilde = y + tau* f( ytilde)
      */
      this->m_rhs->evaluate(y, m_vecf);
      ytilde = y + (tau / 2.0) * m_vecf;
      this->m_rhs->evaluate(ytilde, m_vecf);
      y += tau * m_vecf;
    }
  };
  ...
  public:
        ...
        : TimeStepper(rhs), m_tau(std::make_shared<Parameter>(0.0))
    {
      ...
      m_equ = ynew - m_yold - 0.5 * (m_tau * (m_rhs + m_rhs_old));
    }

    void
    doStep(double tau, VectorView<double> y) override
    {
      m_yold->set(y);
      m_tau->set(tau);
      this->m_rhs->evaluate(y, m_rhs_old->get());
